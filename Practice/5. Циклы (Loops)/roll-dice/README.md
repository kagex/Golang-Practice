...
Если в функцию было передано число `8`, то вывод может выглядеть следующим образом (количество бросков может быть разным, в зависимости от того, какие случайные числа выпадут):
...
- В конце каждой строки должен быть символ переноса строки.
- Некоторые слова в конечном сообщении склоняются, в зависимости от числа, которое отражает количество бросков (например: *1 бросок*, *2 броска*, *5 бросков*).
